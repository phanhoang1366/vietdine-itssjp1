...
export default function SearchOverlay({ onSelect }: SearchOverlayProps) {
  const popularSearches = [
    { label: '寿司' },
    { label: 'フォー' },
    { label: 'WiFiあり', icon: true },
    { label: 'ランチ' },
    { label: 'テラス席' },
  ];

  const popularCategories = [
    { label: '寿司', image: '/images/sushi.png', icon: 'restaurant' },
    { label: 'カフェ', image: '/images/cafe.png', icon: 'local_cafe' },
    { label: '伝統料理', image: '/images/traditional.png', icon: 'festival' },
  ];

  const recentSearches = [
    { title: 'ハノイフォー 専門店', subtitle: 'カテゴリー: ベトナム料理', time: '10分前' },
    { title: '隠れ家 カフェ', subtitle: 'カテゴリー: カフェ / スイーツ', time: '2時間前' },
    { title: '六本木 高級寿司', subtitle: '場所: 東京都港区', time: '昨日' },
  ];
    ...
    <div className="max-w-5xl mx-auto flex gap-12 px-6 pt-6">
      ...
      <div className="w-[360px] bg-surface-container-low rounded-3xl p-6 flex flex-col">
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="text-xs text-on-surface-variant hover:text-primary">削除</button>
        </div>
