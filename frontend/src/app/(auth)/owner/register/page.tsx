      ...
      <div className="hidden lg:flex lg:w-[48%] relative overflow-hidden">
        ...
        <div className="absolute bottom-16 left-8 right-8 z-20">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-secondary-fixed">Owner Portal</span>
          </div>
          <h2 className="text-4xl font-extrabold text-white leading-tight mb-4">ビジネスを<br />成長させよう。</h2>
          <p className="text-white/70 text-sm leading-relaxed max-w-sm">日本人コミュニティに直接アプローチし、あなたのレストランを最大限にアピールしましょう。</p>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-y-auto bg-surface">
        ...
        <div className="hidden lg:flex justify-end px-8 py-6">
          <Link href="/login" className="flex items-center gap-2 px-4 py-2 rounded-full bg-surface-container-high text-on-surface-variant text-xs font-bold hover:bg-surface-container transition-colors">
            <span className="material-symbols-outlined text-sm">person</span>ユーザーログイン
          </Link>
        </div>
        <div className="flex-1 flex items-center justify-center px-6 lg:px-16 py-8">
          <div className="w-full max-w-lg">
            <div className="mb-10">
              <p className="text-secondary font-bold text-sm tracking-wide mb-2">オーナー登録</p>
              <h1 className="text-3xl font-extrabold text-primary tracking-tight mb-2">店舗アカウント作成</h1>
              <p className="text-on-surface-variant text-sm">VietDineでビジネスを始めましょう。</p>
            </div>
            {state?.message && (
            ...
            <form action={action} className="space-y-5">
              <AuthInput id="name" name="name" label="オーナー名" placeholder="例：佐藤 花子" error={state?.errors?.name} />
              <AuthInput id="restaurantName" name="restaurantName" label="レストラン名" placeholder="例：フォー・ハノイ" error={state?.errors?.restaurantName} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <AuthInput id="email" name="email" label="メールアドレス" type="email" placeholder="owner@restaurant.com" error={state?.errors?.email} />
                <AuthInput id="phone" name="phone" label="電話番号" placeholder="090 1234 5678" error={state?.errors?.phone} />
              </div>
              <AuthInput id="password" name="password" label="パスワード" type="password" placeholder="••••••••" error={state?.errors?.password} />
              <AuthInput id="confirmPassword" name="confirmPassword" label="パスワード（確認用）" type="password" placeholder="••••••••" error={state?.errors?.confirmPassword} />
              <div className="bg-surface-container-low rounded-xl p-4">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-xs text-on-surface-variant leading-relaxed">
                    <a href="#" className="font-bold text-on-surface hover:underline">利用規約</a>および<a href="#" className="font-bold text-on-surface hover:underline">プライバシーポリシー</a>に同意します。
                  </span>
                </label>
              </div>
              <AuthButton text="オーナー登録する" pending={pending} />
            </form>
            <p className="mt-8 text-center text-sm font-medium text-on-surface-variant">
              すでにオーナーアカウントをお持ちですか？
              <Link className="text-secondary font-bold hover:underline ml-1" href="/owner/login">ログイン</Link>
            </p>
